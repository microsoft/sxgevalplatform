...
<#
.SYNOPSIS
    Production Deployment Script for East US 2 Region
.DESCRIPTION
    Deploys the evaluation platform to the East US 2 production region
#>
...
$ContainerAppName = "eval-runner-container-prod-eus2"
$RegistryName = "evalplatformregistryprod"
$ImageName = "eval-runner-app"
$Timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$ImageTag = "${RegistryName}.azurecr.io/${ImageName}:prod-eastus2-${Timestamp}"

# Logging setup
$LogFile = "deployment-prod-eastus2-${Timestamp}.log"
...
function Build-DockerImage {
    ...
    try {
        ...
        az acr build `
            ...
            --image "${ImageName}:prod-eastus2-${Timestamp}" `
            --image "${ImageName}:prod-eastus2-latest" `
            --file Dockerfile `
            --platform linux `
            .
...
function Update-ContainerApp {
    ...
    try {
        ...
        # Update environment variables in batches to avoid command line length limits
        Write-Log "Updating $($EnvVars.Count) environment variables in batches" "INFO"
        
        $batchSize = 10
        $allKeys = @($EnvVars.Keys)
        $batches = [Math]::Ceiling($allKeys.Count / $batchSize)
        
        for ($i = 0; $i -lt $batches; $i++) {
            $startIndex = $i * $batchSize
            $endIndex = [Math]::Min(($startIndex + $batchSize - 1), ($allKeys.Count - 1))
            $batchKeys = $allKeys[$startIndex..$endIndex]
            
            Write-Log "Updating batch $($i + 1) of $batches ($($batchKeys.Count) variables)" "INFO"
            ...
            foreach ($key in $batchKeys) {
                ...
                if ($null -ne $value -and $value -ne "") {
                    ...
                    $envArgs += "${key}=`"${escapedValue}`""
                }
            }
            
            if ($envArgs.Count -gt 0) {
                $envArgsString = $envArgs -join " "
                $command = "az containerapp update --name $ContainerAppName --resource-group $ResourceGroup --set-env-vars $envArgsString"
                Invoke-Expression $command
                
                if ($LASTEXITCODE -ne 0) {
                    throw "Container app environment variables update failed at batch $($i + 1)"
                }
            }
        }
...
try {
    Write-Log "========================================" "INFO"
    Write-Log "Starting Production Deployment - East US 2" "INFO"
    Write-Log "========================================" "INFO"
    ...
    if (-not $SkipBuild) {
        ...
        $imageTag = "${RegistryName}.azurecr.io/${ImageName}:prod-eastus2-latest"
    }
    ...
    if ($status -eq "Running") {
        ...
        Write-Log "Region: East US 2" "INFO"
