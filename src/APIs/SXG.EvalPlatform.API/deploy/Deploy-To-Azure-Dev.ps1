...
# Step 8: Post-Deployment Environment Variable Verification and Update
Write-Host "🔧 Post-Deployment: Ensuring ASPNETCORE_ENVIRONMENT is set to Production..." -ForegroundColor Yellow

# Get current environment setting
$currentEnvSetting = az webapp config appsettings list `
    --name $AppName `
    --resource-group $ResourceGroupName `
    --query "[?name=='ASPNETCORE_ENVIRONMENT'].value" -o tsv

Write-Host "Current ASPNETCORE_ENVIRONMENT: $currentEnvSetting" -ForegroundColor Gray

# Always update to Production regardless of current value
...
$updatedEnvSetting = az webapp config appsettings list `
    --name $AppName `
    --resource-group $ResourceGroupName `
    --query "[?name=='ASPNETCORE_ENVIRONMENT'].value" -o tsv

if ($updatedEnvSetting -eq "Production") {
    Write-Host "✅ ASPNETCORE_ENVIRONMENT successfully set to Production" -ForegroundColor Green
} else {
    Write-Host "⚠️ Warning: ASPNETCORE_ENVIRONMENT is set to: $updatedEnvSetting" -ForegroundColor Yellow
}

# Restart the app to ensure new environment variables take effect
Write-Host "🔄 Restarting app to apply environment variable changes..." -ForegroundColor Yellow
az webapp restart `
    --name $AppName `
    --resource-group $ResourceGroupName `
    --output table
...
Write-Host "⏳ Waiting for app to warm up after restart..." -ForegroundColor Gray
Start-Sleep -Seconds 15

# Step 9: Create and Upload Default Configuration (if not exists)
...
Write-Host ""
Write-Host "🏭 Environment: Production (ASPNETCORE_ENVIRONMENT=Production)" -ForegroundColor Green
...
try {
    ...
    # Verify environment in health response
    if ($healthResponse.environment -eq "Production") {
        Write-Host "✅ Environment confirmed as Production in health response" -ForegroundColor Green
    } else {
        Write-Host "⚠️ Warning: Health response shows environment as: $($healthResponse.environment)" -ForegroundColor Yellow
    }
} catch {
...
Write-Host "5. Monitor Application Insights for telemetry and performance" -ForegroundColor White
Write-Host "6. Verify environment setting: https://$appUrl/api/v1/health" -ForegroundColor White

Write-Host ""
Write-Host "🎯 Deployment completed successfully with Production environment! 🎉" -ForegroundColor Green