    ...
    public class RedisCacheManager : ICacheManager
    {
        ...
        public async Task<T?> GetAsync<T>(string key, CancellationToken cancellationToken = default) where T : class
        {
            ...
            try
            {
                ...
                // Debug shortcut: if we have a direct connection multiplexer, try it first to isolate auth/connect issues
                if (_connectionMultiplexer != null)
                {
                    try
                    {
                        var dbDirect = _connectionMultiplexer.GetDatabase();
                        var raw = await dbDirect.StringGetAsync(key);
                        if (raw.HasValue)
                        {
                            Interlocked.Increment(ref _hitCount);
                            var cachedJson = (string)raw;
                            var cachedValue = JsonSerializer.Deserialize<T>(cachedJson, _jsonOptions);

                            activity?.SetTag("cache.hit", true);
                            activity?.SetTag("cache.data_size", cachedJson.Length);
